...
our_pick_model_path = '/data/jhkim/iros23/OFA_refcoco_checkpoints_0208_pick/'
pick_model_path_dict = {
    #'0': '/data/jhkim/iros23/OFA_checkpoints/refcoco_large_best.pt',
    #'8': our_pick_model_path + 'train_82e-5/checkpoint_best.pt',
    #'33': our_pick_model_path + 'train_332e-5/checkpoint_last.pt',
    '135': our_pick_model_path + '0208_train_135/checkpoint_last.pt',
    #'540': our_pick_model_path + 'train_5402e-5/checkpoint_last.pt'
    #'pq': '/data/jhkim/iros23/OFA_refcoco_checkpoints_pseudoQ/0208_train_135/checkpoint_last.pt'
}

our_place_model_path = '/data/jhkim/iros23/OFA_refcoco_checkpoints_0208_place/'
place_model_path_dict = {
    #'0': '/data/jhkim/iros23/OFA_checkpoints/refcoco_large_best.pt',
    #'8': our_place_model_path + 'place_train_8/checkpoint_best.pt',
    #'33': our_place_model_path + 'place_train_33/checkpoint_best.pt',
    '135': our_place_model_path + 'place_0208_train_135/checkpoint_last.pt',
    #'540': our_place_model_path + 'place_train_540/checkpoint_best.pt'
    #'pq': '/data/jhkim/iros23/OFA_refcoco_checkpoints_pseudoQ_place/place_0208_train_135/checkpoint_last.pt'
}
...
for model_num, model_path in pick_model_path_dict.items():
    ...
    #if globals()['pick_cfg_{}'.format(model_num)].common.seed is not None and not globals()['pick_cfg_{}'.format(model_num)].generation.no_seed_provided:
    #    np.random.seed(globals()['pick_cfg_{}'.format(model_num)].common.seed)
    #    utils.set_torch_seed(globals()['pick_cfg_{}'.format(model_num)].common.seed)
...
pnp_image_root_path = '/data/jhkim/iros23/images/pnp_test/'
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!!!!!!!!To Do!!!!!!!!!!!!!!
#query_path = '../pnp_instruction.pth'
#query_list = torch.load(query_path)
#query_list = 
#query_list = [['pick the leftmost cup', 'place it in front of the green cup'],['fetch the right blue cup','place it on the right side of the banana']]
#[[0th pick, 0th place], [1st pick, 1st place], ...]
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
HOST = '147.47.200.155'
PORT = 9998
...
while True:
    ...
    try:
        ...
        model_in = 0
        while model_in == 0:
            #model_name = input("Insert model name [0,8,33,135,540,pq] ... ")
            model_name = '135'
        ...
        query_in = 0
        while query_in == 0:
            query_num = input("Insert instruction number [0 ~ 67] ... ")
            if int(query_num) in range(len(query_list)):
                print("instruction number {}\n pick instruction: {}\n place instruction: {}\n".format(query_num, query_list[int(query_num)][0], query_list[int(query_num)][1]))
                query_in = 1
            else:
                print("wrong query number!")
                '''
        # image processing ... crop, resize, cvt color, save
        #image = Image.open(pnp_image_root_path+'0.png')
        image = Image.open(save_image_path)
        image = np.asarray(image)
        #source = cv2.resize(source, None, fx=0.4, fy=0.4, interpolation=cv2.INTER_LINEAR)
        ...
        #pick_instruction = query_list[int(query_num)][0]
        #place_instruction = query_list[int(query_num)][1]
